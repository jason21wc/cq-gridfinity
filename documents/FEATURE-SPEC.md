...
| 1B.11 | Fit-to-drawer baseplate | `gridfinity-rebuilt-baseplate.scad` | `[Fit to Drawer]` section: `distancex/y`, `fitx/fity` | 1B | Verified | `distancex/y` = target drawer dimension (mm). `length_u/width_u=0` → auto-fill (`max(1, floor(dist/42))`). `fitx/fity` (-1 to 1) controls padding alignment: -1=left, 0=center, 1=right. Outer block = `max(grid, distance)`, grid offset = `padding * fit / 2`. All features (magnets, screws, skeleton, screw-together) compatible. Impl: `gf_baseplate.py` (4 params, 4 properties, 6 methods updated). Tests: `test_baseplate.py` (7 tests). |
...
| 1D.3 | Cullenect click-in label | CullenJWebb/**Cullenect-Labels** (MIT) | `OpenSCAD/Cullenect.scad` | `cullenect_base_v1/v2()`, `cullenect_socket()`, `cullenect_label_generate()` | 1D | Verified | Snap-in label. Width=(42×units)-6mm, height=11mm, depth=1.2mm, latch inset=0.2mm, latch depth=0.6mm. Embossed/debossed text, 18+ hardware icons |
| 1D.4 | Label negative volume | CullenJWebb/**Cullenect-Labels** (MIT) | `OpenSCAD/Cullenect.scad` + `Cullenect Negative Volume.stl` | `cullenect_socket_negative()`, `cullenect_vertical_socket_negative()` | 1D | Verified | Boolean subtraction volume. Socket XY offset=0.3mm, rib=0.2mm XY/0.4mm depth. Generate natively in CadQuery |
| 1D.5 | **Cullenect socket ON the bin** | CullenJWebb/**Cullenect-Labels** (MIT) | applies `cullenect_socket_negative()` to a bin's label shelf | P4 | Verified | `GridfinityBox(cullenect_socket=True)` cuts the negative into the label shelf's flat top face. Dispositioned Keep by Jason 2026-08-16. Tile seats in the real bin with 0.000000 mm3 interference, flush with the shelf |
...
| # | Feature | Source File | Function/Module | Phase | Status | Acceptance |
|---|---------|-------------|-----------------|-------|--------|------------|
| 1E.1 | Clip latch | `rugged-box/rugged-box-library.scad` | `_clip_latch_shape()` (line ~1629), `_clip_latch_part()`, `_latch()` dispatcher, `rbox_latch()` | P3 | Verified | `latch_type="clip"`. Single-piece: hinge eyelet + body + catch w/ flex slot. M3 screws, `latch_edge_radius` 0.8mm, `latch_base_size` 4.5mm. Width: 22mm (generic) / 28mm (GF) |
| 1E.2 | Draw latch | `rugged-box/rugged-box-library.scad` | `_draw_latch_handle()` (~1853), `_draw_latch_catch()` (~1990), `_draw_latch_part()` (~2015) | P3 | Verified | `latch_type="draw"`. Two-piece (handle+catch via pin joint). Body angle 25°, grip angle 45°, thickness 2.25mm, handle length 14.625mm. Segmented interlocking, sep=0.4mm |
| 1E.3 | Lip seal (4 types) | `rugged-box/rugged-box-library.scad` | `_box_seal_shape()` (~991), `_box_seal()`, `_box_add_seal()` | P3 | Verified | `lip_seal_type`: none/wedge/square/filament-1.75mm. Wedge+square: `seal_thickness = total_lip_thickness/3`, protrude top + groove bottom. Filament: 1.75mm circle, groove both halves |
| 1E.4 | Integrated baseplate (GF-specific) | `gridfinity/rugged-box/rugged-box-gridfinity.scad` | `gridfinity_base_plate_style()` (~145), `gridfinity_base_plate_magnets_enabled()`, `gridfinity_base_extra_height()` | P3 | Verified | Shipped as **two booleans** `baseplate_magnets` × `baseplate_skeletonized`, not 4 named styles (triage decision). minimal=F,F (default, matches upstream) · enabled_full=T,F · enabled=T,T. Upstream's "thick" is deliberately unreachable — ballast. Reuses `GridfinityBaseplate`, as upstream reuses kennetek's. Interior grows by the plate's slab depth (upstream `base_extra_height` = h_hole 2.4mm; ours derived, agrees exactly). **Also fixed here: `border = 5` was missing from the interior**, see 1E.8. Stackable offsets (3.4 / -0.8 / -0.6) belong to stackable mode → 1E.5 |
| 1E.5 | Stacking latches | `rugged-box/rugged-box-library.scad` | `_stacking_latch_shape()` (~2079), `_stacking_latch_part()`, `_box_stacking_latch_ribs()` | P3 | Verified | Side-mounted. `catch_offset` -10mm, `grip_length` 8mm, `screw_separation` 20mm. Requires box height >40mm. Stacking gap 1.6mm (GF stackable). **NOTE:** the stackable-plate offsets (3.4 / -0.8 / -0.6mm) recorded here belong to smkent's *Gridfinity Stackable* mode — external plates on the box's outside — which is a DIFFERENT feature and is **not built**. They are not used by the stacking latches |
| 1E.6 | Third hinge | `rugged-box/rugged-box-library.scad` | `rbox_size_adjustments()` (~186), `_box_attachment_placement(hinge=true)` (~1162) | P3 | Verified | Center hinge when `inner_width >= third_hinge_width`. GF: activates at 5U wide (210mm). Adds 1 screw to BOM |
| 1E.7 | Hinge end stops | `rugged-box/rugged-box-library.scad` | `_box_hinge_rib_bottom_end_stop()` (~1400), in `_box_hinge_ribs()` | P3 | Verified | Bottom box part only. Physical rotation limiter. Width = latch_width. Positioned 2mm below box top |
| 1E.14 | Reinforced corners | `rugged-box/rugged-box-library.scad` | `_box_sides()` with `reinforced_corners`, `_box_wall_shape(reinforced=true)` | P3 | Verified | Corners keep full `total_lip_thickness` for their whole height instead of stepping in with the flats; the chamfer is measured from the lip footprint too. **Default true**, matching smkent's Gridfinity wrapper. Dispositioned Keep by Jason 2026-08-16 |
| 1E.9 | Support ribs (side + rear) | `rugged-box/rugged-box-library.scad` | `_box_rib_shape()` (~1059), `_box_rib()` (~1074), `_box_plain_rib()` (~1099), `_box_ribs()` (~1104); GF positions from `rb_side_rib_positions()` / `rb_rear_rib_positions()` in the wrapper | P3 | Verified | Rib cross-section is a trapezoid rounded by `_round_shape(edge_radius)`; `plain_ribs_angle = 8`, plain ribs are `rib_width * 2` wide. Side ribs one per grid unit along Y at ±X; rear ribs on interior grid lines only (`i = 1 .. Width-2`) at +Y |
| 1E.10 | Attachment placement framework | `rugged-box/rugged-box-library.scad` | `_box_attachment_placement()` (~1149), `_box_attachment_rib_pair()` (~1140), `_box_attachment_rib_cut()` (~1121), `_compute_latch_count()` (~543) | P3 | Verified | Shared by latch and hinge. Pair offset `(latch_width + rib_width) / 2`; latch ribs mirror to −Y, hinge ribs stay +Y. **1E.6 lives here** as the extra `[0]` position |
| 1E.11 | Screw eyelets and holes | `rugged-box/rugged-box-library.scad` | `_box_screw_eyelet_body()` (~1171), `_box_screw_hole()` (~1177), `_attachment_screw_offset()` (~539) | P3 | Verified | `screw_eyelet_radius = 3.0 * 3.0 / 2 = 4.5`; offset `total_lip_thickness + eyelet_r + 0.2 = 10.7`. Hole is undersized `-0.1` on the bottom half and oversized `+0.6` on the top so the screw threads into one and turns freely in the other |
| 1E.12 | Latch mounting ribs | `rugged-box/rugged-box-library.scad` | `_box_latch_rib_base()` (~1192), `_box_latch_rib()` (~1233), `_box_latch_ribs()` (~1244), `_latch_offset_from_base()` (~568) | P3 | Verified | What 1E.1/1E.2 bolt to. Screw height differs per half: top uses `latch_amount_on_top`, bottom `latch_screw_separation − latch_amount_on_top` |
| 1E.13 | Hinge ribs (top + bottom) | `rugged-box/rugged-box-library.scad` | `_box_hinge_rib_body()` (~1324), `_box_hinge_ribs_top()` (~1361), `_box_hinge_rib_bottom()` (~1386), `_box_hinge_ribs()` (~1411) | P3 | Verified | Interleaved eyelets on the two halves joined by an M3 acting as the pin. Top eyelet carries `top_hinge_eyelet_position_tolerance = 0.1`; `hinge_size_tolerance = 0.1` between interleaved widths |
| 1E.8 | Parametric walls | `rugged-box/rugged-box-library.scad` | `rbox_size_adjustments()` (~186) | P3 | Verified | 7 params: wall_thickness (2.4/3.0mm), lip_thickness (2.0/3.0mm), rib_width (4/6mm), latch_width (22/28mm), latch_screw_sep (20/16mm), latch_amount_on_top, size_tolerance (0.05/0.20mm). Generic/GF defaults differ |

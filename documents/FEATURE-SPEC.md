...
| 1D.5 | **Cullenect socket ON the bin** *(proposed — not built)* | CullenJWebb/**Cullenect-Labels** (MIT) | applies `cullenect_socket_negative()` to a bin's label shelf | P4 | **Proposed** | 1D.3/1D.4 deliver the tile and the negative volume, but nothing places the socket on a `GridfinityBox`, so a user cannot yet get a bin that accepts a tile. The triage rationale for 1D.3 ("relabel by reprinting a 2g tile, not a 6-hour bin") presupposes this. **Needs Jason's disposition before implementation** |

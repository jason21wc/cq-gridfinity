...
| 1E.9 | Support ribs (side + rear) | `rugged-box/rugged-box-library.scad` | `_box_rib_shape()` (~1059), `_box_rib()` (~1074), `_box_plain_rib()` (~1099), `_box_ribs()` (~1104); GF positions from `rb_side_rib_positions()` / `rb_rear_rib_positions()` in the wrapper | P3 | Not Started | Rib cross-section is a trapezoid rounded by `_round_shape(edge_radius)`; `plain_ribs_angle = 8`, plain ribs are `rib_width * 2` wide. Side ribs one per grid unit along Y at ±X; rear ribs on interior grid lines only (`i = 1 .. Width-2`) at +Y |
| 1E.10 | Attachment placement framework | `rugged-box/rugged-box-library.scad` | `_box_attachment_placement()` (~1149), `_box_attachment_rib_pair()` (~1140), `_box_attachment_rib_cut()` (~1121), `_compute_latch_count()` (~543) | P3 | Not Started | Shared by latch and hinge. Pair offset `(latch_width + rib_width) / 2`; latch ribs mirror to −Y, hinge ribs stay +Y. **1E.6 lives here** as the extra `[0]` position |
| 1E.11 | Screw eyelets and holes | `rugged-box/rugged-box-library.scad` | `_box_screw_eyelet_body()` (~1171), `_box_screw_hole()` (~1177), `_attachment_screw_offset()` (~539) | P3 | Not Started | `screw_eyelet_radius = 3.0 * 3.0 / 2 = 4.5`; offset `total_lip_thickness + eyelet_r + 0.2 = 10.7`. Hole is undersized `-0.1` on the bottom half and oversized `+0.6` on the top so the screw threads into one and turns freely in the other |
| 1E.12 | Latch mounting ribs | `rugged-box/rugged-box-library.scad` | `_box_latch_rib_base()` (~1192), `_box_latch_rib()` (~1233), `_box_latch_ribs()` (~1244), `_latch_offset_from_base()` (~568) | P3 | Not Started | What 1E.1/1E.2 bolt to. Screw height differs per half: top uses `latch_amount_on_top`, bottom `latch_screw_separation − latch_amount_on_top` |
| 1E.13 | Hinge ribs (top + bottom) | `rugged-box/rugged-box-library.scad` | `_box_hinge_rib_body()` (~1324), `_box_hinge_ribs_top()` (~1361), `_box_hinge_rib_bottom()` (~1386), `_box_hinge_ribs()` (~1411) | P3 | Not Started | Interleaved eyelets on the two halves joined by an M3 acting as the pin. Top eyelet carries `top_hinge_eyelet_position_tolerance = 0.1`; `hinge_size_tolerance = 0.1` between interleaved widths |

...
| # | Feature | Source File | Function/Module | Phase | Status | Acceptance |
|---|---------|-------------|-----------------|-------|--------|------------|
| 1B.5 | Scoop scaling (0-1) | `src/core/cutouts.scad` | `cut_compartment_auto()` with `scoop` param | 1B | Not Started | `scoop` param 0.0-1.0 (0.1 step), scales scoop radius. 0=none, 1=full. Entry point: `gridfinity-rebuilt-bins.scad` `scoop = 1; //[0:0.1:1]` |
| 1B.6 | Tab positioning (6 styles) | `src/core/cutouts.scad` | `cut_compartment_auto()` with `style_tab` param | 1B | Not Started | `style_tab`: 0=Full, 1=Auto, 2=Left, 3=Center, 4=Right, 5=None. Plus `place_tab`: 0=Everywhere, 1=Top-Left-Division-Only. Tab angle: `a_tab` (default 40°) |
| 1B.7 | Custom compartment depth | `gridfinity-rebuilt-bins.scad` + `src/core/bin.scad` | `depth` param → `cgs(height=depth)`, `height_internal` → `fill_height` in `new_bin()` | 1B | Not Started | Two modes: `depth` sets compartment floor offset (0=full depth), `height_internal` overrides internal block height (mm). Both passed through `new_bin()` |
| 1B.8 | Cylindrical compartments | `gridfinity-rebuilt-bins.scad` + `src/core/cutouts.scad` | `cut_chamfered_cylinder(cd/2, depth_real, c_chamfer)` | 1B | Not Started | `cut_cylinders=true`, `cd` = diameter (default 10mm), `c_chamfer` = chamfer radius (default 0.5mm). Round compartments cut instead of rectangular |

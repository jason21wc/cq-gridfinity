...
| **Reduced lip recess is not tapered** | — | Found during the 2026-08-13 double-check. `GR_REDUCED_LIP_PROFILE` is `[1.6@45, 1.2, 4.4-straight]` — both trailing segments straight, so the wall is a constant 2.60mm for 5.6mm below the rim. The normal lip tapers from 1.45mm at the rim, which is what makes its recess wide enough to accept a bin's base. **Pre-existing**, not introduced by the spec fix. Consequence: reduced and normal do NOT stack interchangeably despite now being the same height — the recess openings differ by ~1.15mm per side. Decide whether "reduced" should taper at the rim, or be documented as non-stacking |
